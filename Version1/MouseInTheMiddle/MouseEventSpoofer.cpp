...
void MouseEventSpoofer::spoofEvent() {
    ...
    if(hasUSBData) {
        ...
        if (usbMouseButtons & MOUSE_MIDDLE) {
            // Log the data to serial1 so the program can enable scripts
            Serial1.print("MRPT: ");
            Serial1.print(usbMouseButtons, HEX);
            Serial1.println();
        ...
        if (usbMouseButtons & MOUSE_LEFT) {
            if (!(usbPreviousMouseButtons & MOUSE_LEFT)) {
                // LMB was just pressed, send a message for press event
                Serial1.print("MRPT: ");
                Serial1.print(usbMouseButtons, HEX);
                Serial1.println();
            }
        } else if (usbPreviousMouseButtons & MOUSE_LEFT) {
            // LMB was just released, send a message for release event
            Serial1.print("MRPT: ");
            Serial1.print(usbMouseButtons, HEX);
            Serial1.println();
        }
...
void MouseEventSpoofer::handleButtonEvents(uint8_t usbButtons, uint8_t previousUsbButtonsState, uint8_t serialButtons, uint8_t previousSerialButtonsState) {
    ...
    for (uint16_t buttonId = 1; buttonId <= 16; buttonId <<= 1) {
        ...
        switch(buttonId) {
            ...
            default:
                Serial1.println(String("MSG: Unknown ButtonID: ") + buttonId);
                continue;
        }
