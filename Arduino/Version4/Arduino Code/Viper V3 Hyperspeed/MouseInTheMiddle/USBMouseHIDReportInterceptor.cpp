...
{
    processAndSetHIDReportData00B8(buf);
}
...
void USBMouseHIDReportInterceptor::processAndSetHIDReportData00B8(const uint8_t* data)
{
    if (DEBUG_MODE) {
        FTDI_DEVICE.print("I: ");
    ...
    mouseButtons = data[0];
    xMovement = mathAndConversions.parseX_00B8(data);
    yMovement = mathAndConversions.parseY_00B8(data);
    scrollWheel = data[3];
