...
void CompositeHID_::SendReport(uint8_t id, const void* data, int len) {
    Serial1.print("Attempting to send report ID: ");
    Serial1.print(id);
    Serial1.print(" with endpoint ");
    uint8_t ep = (id == REPORT_ID_MOUSE) ? pluggedEndpoint : (pluggedEndpoint + 1);
    Serial1.println(ep);

    auto ret = USB_Send(ep, &id, 1);
    if (ret < 0) {
        Serial1.println("Failed to send report ID to host.");
        return;
    }
    
    ret = USB_Send(ep | TRANSFER_RELEASE, data, len);
    if (ret < 0) {
        Serial1.println("Failed to send report data to host.");
    } else {
        Serial1.println("Report data sent successfully.");
    }
}
...
int CompositeHID_::getInterface(uint8_t* interfaceCount) {
    Serial1.print("Setting up interface #");
    Serial1.println(*interfaceCount);
    ...
    *interfaceCount += 2;  // Two interfaces: one for mouse, one for combined HID
    ...
    if (res < 0) {
        Serial1.println("Failed to send mouse interface");
        return res;
    }
    ...
    if (res < 0) {
        Serial1.println("Failed to send combined keyboard/consumer control/vendor/system interface");
    }
...
int CompositeHID_::getDescriptor(USBSetup& setup) {
    ...
    Serial1.print("Get Descriptor for interface: ");
    Serial1.println(setup.wIndex);
...
bool CompositeHID_::setup(USBSetup& setup) {
    Serial1.print("Setup request: ");
    Serial1.print(setup.bmRequestType, HEX);
    Serial1.print(" ");
