...
void HIDMouseReportParser::Parse(USBHID* hid, bool is_rpt_id, uint8_t len, uint8_t* buf)
{
    ...
    if (buf[0] & 4)
    {
        mmbTimestamp = millis();
        buf[0] &= ~4;
        Serial.println("MSG: MMB Detected, excluding it from the report.");
    }
    ...
    if (!(previousButtonsState & 0x10) && (millis() - mmbTimestamp) <= 1200)
    {
        ...
        Serial.println("MSG: MB5 Detected; excluding it from the mouse report.");
    }

    // TODO: Check if MB4 events should be excluded from the hid report
    ...
    xMovement = razerViperV2Pro16BitDataParser.parseX(buf);
    yMovement = razerViperV2Pro16BitDataParser.parseY(buf);
