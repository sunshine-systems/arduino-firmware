...
int16_t parseX_2022(const uint8_t* data) {
    // X is always a 16-bit little-endian value in bytes 2-3
    return static_cast<int16_t>(data[2] | (data[3] << 8));
}

int16_t parseY_2022(const uint8_t* data) {
    // Y is always a 16-bit little-endian value in bytes 4-5
    return static_cast<int16_t>(data[4] | (data[5] << 8));
}
