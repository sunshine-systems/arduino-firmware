...
public:
  ...
  int16_t parseY_00A6(const uint8_t* data) {
    ...
    int16_t parseX_F508(const uint8_t* data) {
      int16_t x;
      uint8_t overflowByte = data[2];

      if (overflowByte == 0x80 || overflowByte == 0x7F) {
        x = twosComplement_00A6(data[1], data[2]);
      } else {
        x = static_cast<int16_t>(data[1]);
        if (x & (1 << 7)) {
          x -= 1 << 8;
  ...
  int16_t parseY_F508(const uint8_t* data) {
    int16_t y;
    uint8_t overflowByte = data[4];

    if (overflowByte == 0x80 || overflowByte == 0x7F) {
      y = twosComplement_00A6(data[3], data[4]);
    } else {
      y = static_cast<int16_t>(data[3]);
      if (y & (1 << 7)) {
        y -= 1 << 8;
      }
    }
